...
See the vignette [Treating Color Image with imagerExtra](https://cran.r-project.org/package=imagerExtra/vignettes/treat_colorimage.html) if you want to treat color image with imagerExtra.
...
```{r, fig.width=7, fig.height=7}
layout(matrix(1:2,1,2))
ThresholdML(g, 3) %>% plot(main = "k = 3")
ThresholdML(g, thr = c(0.2, 0.4, 0.6)) %>% plot(main = "thr = c(0.2, 0.4, 0.6)")
```
...
See the vignette [Treating Color Image with imagerExtra](https://cran.r-project.org/package=imagerExtra/vignettes/treat_colorimage.html) for detail.

### OCR and OCR_data

These functions are shortcuts to the ocr function and ocr_data function of the R package tesseract.

See the vignette [Optical Character Recognition with imagerExtra](https://cran.r-project.org/package=imagerExtra/vignettes/OCR_with_imagerExtra.html) for detail.